<script lang="ts">
  ...
  const getList = async (page: number): Promise<CardContent[]>  => {
...
<div class="cardsContainer">
  ...
  {:then results} 
    ...
    {/each}
  {:catch error}
    <h1 style="color: red;">Failed to load results: {`${error?.code ?? 500}: ${error?.message ?? "Internal Server Error"}`}</h1>
  {/await}
</div>
<div class="footer">
  <img src={DuckNavigation} alt="Prev Page" height="48px" on:click={currentPage>0 ? () => currentPage-- : () => {}} on:keypress />
  {currentPage}
  <img src={DuckNavigation} alt="Next Page" height="48px" on:click={currentList.length > 0 ? () => currentPage++ : () => {}} on:keypress />
</div>